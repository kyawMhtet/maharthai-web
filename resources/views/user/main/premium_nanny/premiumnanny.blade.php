...
@section('content')
    ...
    <div class="filter container w-25 mt-5">
        <div class="filter1 shadow-sm py-2 rounded text-danger">
            <form action="{{ route('premiumnanny#search') }}" method="get">
                @csrf
                <div class="d-flex ">
            ...
            @foreach ($premiumnannies as $premiumnanny)
                @if ($premiumnanny->stockstatus == 'Available' || $premiumnanny->stockstatus == 'null' || $premiumnanny->stockstatus == '')
                    <div class="col-12 col-xl-3 col-lg-3 col-md-6 col-sm-6 ist-item  mt-2 text-center">
                        <div class="premiumnanny card listitem p-2 border border-0">

                            <img class="card-img-top" src="{{ asset('storage/' . $premiumnanny->photo) }}" alt="">
