...
@section('content')
    ...
    {{-- <div class="filter w-25 container mt-5">
        <div class="filter1 shadow-sm py-2 rounded text-danger">
            <form action="{{ route('maideldercare#search') }}" method="post">
                @csrf
                <div class="d-flex">
    ...
    <div class="list container w-75 text-center mb-3">
        ...
        <div class="mt-2">
            <a href="{{ route('mainmaideldercare#page') }}">
        ...
        <div class="row m-auto" id="maideldercareList">
            @foreach ($maideldercares as $maideldercare)
                @if ($maideldercare->stockstatus == 'Available' ||
                    $maideldercare->stockstatus == 'null' ||
                    $maideldercare->stockstatus == '')
                    <div class="col col-lg-4 col-md-6 col-sm-8 mt-5 text-center">
                        ...
                        <div class="maideldercare card p-2 border border-0" style="width: 20rem; height: 600px;">
                            ...
                            <img class="card-img-top" src="{{ asset('storage/' . $maideldercare->photo) }}"
                            ...
                            <div class="text-center mb-4">
                                <a href="{{ route('maideldercare#info', $maideldercare->id) }}"
    ...
    // $.ajax({
    //     type : 'get' ,
    //     url : 'http://localhost/maharthai/public/ajax/maideldercare/list' ,
    //     dataType : 'json' ,
    ...
    $('.workingtime1').change(function() {
        ...
        if ($eventOption1 == 'live-In') {
            $.ajax({
                type: 'get',
                url: '{{ route('filter#maideldercarelist') }}',
                data: {
                    'status': 'live-In'
                },
                dataType: 'json',
