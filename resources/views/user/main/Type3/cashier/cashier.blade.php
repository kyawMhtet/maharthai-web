...
@section('content')
            ...
            <form action="{{ route('cashier#search') }}" method="get">
                ...
                <div class="mt-3 text-center">
                    ...
                    <button class="btn btn-primary">
                        <a href="{{ route('maincashier#page') }}" class="clear">
                            Clear
                        </a>
                    </button>
                </div>
    ...
    <div class="list container text-center mb-3">
        ...
        <div class="row m-auto" id="cashierList">
            @foreach ($cashiers as $cashier)
                @if ($cashier->stockstatus == 'Available' || $cashier->stockstatus == 'null' || $cashier->stockstatus == '')
                    <div class="col-12 col-xl-3 col-lg-3 col-md-6 col-sm-6 ist-item  mt-2 text-center">
                        <div class="cashier card listitem p-2 border border-0">

                            <img class="card-img-top" src="{{ asset('storage/' . $cashier->photo) }}" alt="">
                            <div class="card-body text-start">
                                <div>
                                    <p>{{ $cashier->code }}</p>
                                    <p>{{ $cashier->full_part }} / {{ $cashier->live_in_out }}</p>
                                    <p>{{ $cashier->salary }} <b>Baht</b></p>


                                    <p>{{ Str::words($cashier->experience, 8, '...') }}</p>
                                </div>

                            </div>
                            <div class="text-center mb-4">
                                <a href="{{ route('cashier#info', $cashier->id) }}" class="btn btn-danger rounded-pill w-75">
                                    More Details
                                </a>
                            </div>

                            {{-- <a href="javascript:void(0)" id="show-worker" data-url="{{ route('cashier#info', $cashier->id) }}" class="btn btn-danger">More Details</a> --}}

                        </div>
                    </div>
                @elseif ($cashier->stockstatus == 'Not Available')
                @endif
            @endforeach
        </div>
        <div class="mt-5">
    ...
    <div class="text-center mt-3 text-danger">
        <h2>HIRE FOR CASHIER WORKERS</h2>
    </div>
