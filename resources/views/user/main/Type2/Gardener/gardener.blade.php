...
@section('content')
    ...
    <div class="filter container w-25 mt-5">
        <div class="filter1 shadow-sm py-2 rounded text-danger">
            <form action="{{ route('gardener#search') }}" method="get">
                @csrf
                <div class="d-flex">
                ...
                <div class="mt-3 text-center">
                    ...
                    <button class="btn btn-primary">
                        <a href="{{ route('maingardener#page') }}" class="clear">
                            Clear
                        </a>
                    </button>
                </div>
    ...
    <div class="list container text-center mb-3">
        ...
        <div class="row m-auto" id="gardenerList">
            @foreach ($gardeners as $gardener)
                @if ($gardener->stockstatus == 'Available' || $gardener->stockstatus == 'null' || $gardener->stockstatus == '')
                    <div class="col-12 col-xl-3 col-lg-3 col-md-6 col-sm-6 ist-item  mt-2 text-center">
                        <div class="gardener card listitem p-2 border border-0">

                            <img class="card-img-top" src="{{ asset('storage/' . $gardener->photo) }}" alt="">
                            <div class="card-body text-start">
                                <div>
                                    <p>{{ $gardener->code }}</p>
                                    <p>{{ $gardener->full_part }} / {{ $gardener->live_in_out }}</p>
                                    <p>{{ $gardener->salary }} <b>Baht</b></p>


                                    <p>{{ Str::words($gardener->experience, 8, '...') }}</p>
                                </div>

                            </div>
                            <div class="text-center mb-4">
                                <a href="{{ route('gardener#info', $gardener->id) }}" class="btn btn-danger rounded-pill w-75">
                                    More Details
                                </a>
                            </div>

                            {{-- <a href="javascript:void(0)" id="show-worker" data-url="{{ route('gardener#info', $gardener->id) }}" class="btn btn-danger">More Details</a> --}}
